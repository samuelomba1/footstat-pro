...
function Scores({compId,season}) {
  ...
  if(!data.length) return <div style={{textAlign:"center",padding:"2.5rem",color:T.text2}}>Aucun match disponible.</div>
  return(
    <div>
      <div style={{display:"flex",gap:6,marginBottom:"1rem",flexWrap:"wrap",alignItems:"center"}}>
        ...
        <div style={{marginLeft:"auto",fontFamily:"'JetBrains Mono',monospace",fontSize:".65rem",color:T.text3}}>↻ Auto 60s</div>
      </div>
      {shown.length===0
        ?<div style={{textAlign:"center",padding:"1.5rem",color:T.text2}}>Aucun match.</div>
        :shown.map(f=>{
          const s=f.fixture.status.short
          ...
          return(
            <div key={f.fixture.id} style={{display:"flex",alignItems:"center",gap:".75rem",padding:".75rem 1.1rem",background:T.card,border:`1px solid ${live?T.green+"40":T.border}`,borderRadius:11,marginBottom:".45rem"}}>
...
function Standings({compId,season}) {
  ...
  if(loading) return <div style={{display:"flex",justifyContent:"center",padding:"3rem"}}><Spin size={32}/></div>
  if(!data) return <div style={{textAlign:"center",padding:"2.5rem",color:T.text2}}>Classement non disponible.</div>
  return(
                ...
                {group.map((t,i)=>{
                  ...
                  return(
                    <tr key={t.team.id} style={{borderBottom:`1px solid ${T.border}18`,background:i%2===0?`${T.bg2}40`:"transparent"}}>
                      ...
                      <td style={{padding:".48rem .5rem",textAlign:"center",fontFamily:"'JetBrains Mono',monospace",fontSize:".78rem",color:diff>0?T.green:diff<0?T.red:T.text2}}>{diff>0?"+":""}{diff}</td>
...
function Scorers({compId,season}) {
  ...
  if(loading) return <div style={{display:"flex",justifyContent:"center",padding:"3rem"}}><Spin size={32}/></div>
  if(!data.length) return <div style={{textAlign:"center",padding:"2.5rem",color:T.text2}}>Données non disponibles.</div>
...
function Analyse({compId,compName,season}) {
  ...
  async function analyze() {
    ...
    try {
      ...
      setStep("🤖 Claude AI analyse...")
      const iA=teams.find(t=>t.team.id==teamA)
      const iB=teams.find(t=>t.team.id==teamB)
      const ai=await claudeAnalyze({teamA:iA?.team?.name,teamB:iB?.team?.name,statsA:sA,statsB:sB,h2h:Array.isArray(h2h)?h2h:[],competition:compName,season})
      setResult({sA,sB,h2h:Array.isArray(h2h)?h2h:[],iA,iB,ai})
  ...
  return(
    <div>
      <Card style={{marginBottom:"1.2rem"}}>
        ...
        {tLoad?<div style={{display:"flex",alignItems:"center",gap:10,color:T.text2,fontSize:".85rem"}}><Spin size={18}/>Chargement...</div>:(
          ...
          <div style={{animation:"fadeUp .35s ease"}}>
            <div style={{background:"linear-gradient(135deg,#061309,#08101A)",border:`1px solid ${T.green}28`,borderRadius:15,padding:"1.6rem",marginBottom:"1.2rem",position:"relative",overflow:"hidden"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"1.1rem",flexWrap:"wrap",gap:8}}>
                <span style={{fontFamily:"'Rajdhani',sans-serif",fontWeight:700,fontSize:"1.15rem",color:T.green}}>🤖 Analyse Claude AI</span>
                ...
                <Card key={i}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:".9rem"}}>
                    ...
                    <div>
                      ...
                      <div style={{fontSize:".72rem",color:T.text2}}>{t.s?.fixtures?.played?.total||0} matchs</div>
                    </div>
                  </div>
                  <Sec>Forme récente</Sec>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:".5rem"}}>
                    {[
                      ...
                      {label:"Encaissés",val:t.s?.goals?.against?.average?.total||"—",col:T.red},
            ...
            {h2h.length>0&&(
              <Card>
                <Sec>🔁 CONFRONTATIONS DIRECTES</Sec>
                {h2h.map((f,i)=>{
          ...
          <div style={{flex:1,overflowY:"auto",padding:"1.2rem 1.4rem"}}>
            {!comp?(
              <div style={{textAlign:"center",padding:"5rem 2rem",color:T.text2}}>
                ...
                <p style={{fontSize:".9rem"}}>Sélectionnez une compétition dans la sidebar.</p>
              </div>
            ):(
              <>
                <div style={{marginBottom:"1.2rem"}}>
                  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:".35rem",flexWrap:"wrap"}}>
                    ...
                    {comp.nat&&<Tag color={T.purple}>🌍 Nationales</Tag>}
                  </div>
                  <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:".68rem",color:T.text3}}>Saison {season}/{season+1} · API-Football · Claude AI</div>
                </div>
